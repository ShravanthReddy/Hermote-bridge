...
The bridge that connects the Hermote iPhone app to a Hermes Agent installation on a Mac or Linux host. It supports file uploads up to 256 MiB per file and up to eight terminals per phone connection.
...
Set `HERMOTE_BRIDGE_NO_UP=1` when installing to skip automatic guided setup. On macOS, run `hermote-bridge up` when you are ready to pair.
...
`up`, `restart`, `stop`, and `uninstall` are macOS-only service commands. For a non-interactive macOS installation, run `hermote-bridge up` afterward to complete setup.

## Attachment limits

The bridge has three distinct attachment paths:

- **Inline fallback:** 12 MiB raw as a preflight heuristic. A 12 MiB source may still exceed the JSON, data URL, or envelope budget.
